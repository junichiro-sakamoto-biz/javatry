...
public class Step41DependencyInjectionBeginnerTest extends PlainTestCase {
    ...
    public void test_whatis_DependencyInjection() {
        ...
        // - - - - - (your answer?)
        // コンポーネントのインスタンスの生成や、依存関係の解決をソースコードから分離する。
        // 基本的には注入するコンポーネントにアノテーションをつけて、注入されるコンポーネントのコンストラクタらか実態化されたコンポーネントを受け取る.
        //
        // 疑問
        // 実際の実行時には、アノテーションをつけたコンポーネントがDIコンテナで管理されるみたい
        // その後どうやって注入されるコンポーネントを生成するときにDIコンテナが干渉してコンポーネントを注入しているのかがわからなかった
        //
        // _/_/_/_/_/_/_/_/_/_/
    }
    ...
    public void test_nondi_difference_between_FactoryMethod_and_IndividualFactory() {
        // your answer? => 
        // and your confirmation code here freely
        // TooLazyDogとsupercarDealerを生成する処理をさらに別クラス内の関数に切り出している。
    }
